...
func (c *Client) CreateKeySets(s *vault.Secret) error {
	...
	resp, err := c.gc.TrustFramework().KeySets().Get(context.Background(), nil)
	if err != nil {
		log.Fatal(err)
	}
...
func (c *Client) uploadCertificate(id, cert, pw string) error {
	...
	ks.SetId(&id)
	_, err := c.gc.TrustFramework().KeySets().Post(context.Background(), ks, nil)
	if err != nil {
		return err
	}
	...
	k.SetKey(&cert)
	k.SetPassword(&pw)
	_, err = c.gc.TrustFramework().KeySets().ByTrustFrameworkKeySetId(id).UploadPkcs12().Post(context.Background(), k, nil)
...
func (c *Client) createKeySet(id, use string) error {
	...
	kty := "RSA"
	k.SetKty(&kty)
	_, err = c.gc.TrustFramework().KeySets().ByTrustFrameworkKeySetId(id).GenerateKey().Post(context.Background(), k, nil)
...
func (c *Client) DeleteKeySets() error {
	...
	for _, ks := range resp.GetValue() {
		id := ks.GetId()
		if a.Has(*id) {
			err = c.gc.TrustFramework().KeySets().ByTrustFrameworkKeySetId(*id).Delete(context.Background(), nil)
			if err != nil {
				return err
			}
		}
